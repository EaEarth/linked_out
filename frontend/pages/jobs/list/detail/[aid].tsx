...
export const Response = (props) => {
  ...
  const handleSubmitClick = async (status) => {
    const payload = {
      ...
      coverLetterId: application.coverLetter
    ...
    try {
      await axios
        .patch(`/job-application/${application.id}`, payload)
        .then((response) => {
          if (response.status === 200) {
            router.push(
              `/jobs/list/applicant/${application.jobAnnouncement.id}`
            );
          }
        });
    } catch (err) {
  ...
  const handleChatClick = async (e) => {
    ...
    try {
      await axios.post('/chat/chat-room', payload).then((response) => {
  ...
  return (
    <DefaultLayout>
      <Head>
        <title>
          Response - {application.jobAnnouncement.company} -{' '}
          {application.jobAnnouncement.title}
        </title>
      </Head>
      ...
      <Container className="my-4">
        <Row>
          <Col className="text-center">
            ...
            <h3 className="mt-3">
              {application.jobAnnouncement.company} -{' '}
              {application.jobAnnouncement.title}
            </h3>
          </Col>
        </Row>
        <Row>
          <Col md={{ span: 5, offset: 1 }} className="mt-5 text-center">
            <Image
              src={
                application.applicant.avatarFile?.path ||
            ...
            />
            <button
              className="float-center my-2 btn btn-success"
          ...
          <Col
            md={{ span: 5 }}
            className={`shadow-sm mx-auto mt-5 my-4 ${style['information']}`}>
            <h2 className="mt-2">Information</h2>
            <Row className="mx-auto">
              <Col md={{ span: 3 }} className="font-weight-bold pl-0">
                First Name
              </Col>
            ...
            <Row className="mx-auto">
              ...
              <Col>
                {dayjs().diff(dayjs(application.applicant.birthDate), 'year')}
              </Col>
            </Row>
            <Row className="mx-auto">
              <Col md={{ span: 3 }} className="font-weight-bold pl-0">
            ...
            <p className="">
              {application.education == '' ? '-' : application.education}
            </p>
            {application.resume && (
              <Row className="d-block mx-auto">
                {' '}
                <p className="font-weight-bold my-2">Resume</p>
            ...
            {application.coverLetter && (
              <Row className="d-block mx-auto">
                {' '}
                <p className="font-weight-bold my-2">Cover Letter</p>
            ...
            {application.transcript && (
              <Row className="d-block mx-auto">
                {' '}
                <p className="font-weight-bold my-2">transcript</p>
            ...
            <Row className="mx-auto">
              ...
              <h4
                className={
                  'my-2 text-' + statusColorHandler(application.status)
                }>
                {statusHandler(application.status)}
              </h4>
            </Row>
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const { data } = await axios.get<ApplicationForm[]>(
    `/job-application/index/${context.params.aid}`,
    {
      headers: {
