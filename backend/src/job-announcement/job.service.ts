...
export class JobService {
  constructor(
    ...
    private readonly repo: Repository<JobAnnouncement>,
  ...
  search(dto: searchAnnouncement): SelectQueryBuilder<JobAnnouncement> {
    ...
    for (var i = 0; i < search.length; ++i) {
      ...
      qb = qb.andWhere('(:check like :isNull OR tag.name IN ( :name ))', {
        name: info.tag,
        check: 'true',
        isNull: 'tag' in dto && dto['tag'].length > 0 ? 'false' : 'true',
      }); // must contain one of tags
      qb = qb.andWhere('province like :province', {
        province: `%${info.province}%`,
      }); // must in specific province
      qb = qb.andWhere('upperBoundSalary >= :salary', {
        salary: info.lowerBoundSalary,
      }); // upperBound salary must >= specific salary
  ...
  async update(
    ...
    await this.repo
      .findOne(id, { relations: ['owner'] })
      .then(async (entity) => {
        if (entity == undefined)
          throw new NotFoundException('Job announcement not found');
        if (
          !ability.can(Action.Update, entity) &&
          !(owner.id == entity.owner.id)
        )
  ...
  async delete(owner: User, id: number): Promise<JobAnnouncement> {
    const ability = this.caslAbilityFactory.createForUser(owner);
    const jobAnnouncement = await this.repo.findOne(id, {
      relations: ['owner'],
    });
    if (jobAnnouncement == undefined)
      throw new NotFoundException('Job announcement not found');
    if (
      !ability.can(Action.Update, jobAnnouncement) &&
      !(owner.id == jobAnnouncement.owner.id)
    )
      throw new UnauthorizedException();
  ...
  private prepareSearch(dto: searchAnnouncement) {
    return {
      ...
      lowerBoundSalary:
        'lowerBoundSalary' in dto && dto['lowerBoundSalary'] !== null
          ? dto['lowerBoundSalary']
          : 0,
      province:
        'province' in dto && dto['province'] !== null ? dto['province'] : '',
  ...
  async recommendedJob(id: number): Promise<JobAnnouncement[]> {
    ...
    let info: searchAnnouncement = {
      tag: user.tags.map((a) => a.name),
